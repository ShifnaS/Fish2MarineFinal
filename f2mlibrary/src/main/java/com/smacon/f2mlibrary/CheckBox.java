...
/**
 * Created by Aiswarya on 11/30/2016.
 */
public class CheckBox extends View implements Checkable {
    ...
    public CheckBox(Context context, AttributeSet attrs) {
        ...
        if(attrs != null){
            ...
            int colorAfter = array.getColor(R.styleable.BBox_color_after, Color.WHITE);
            int colorBefore = array.getColor(R.styleable.BBox_color_before,Color.WHITE);
        ...
        mPaintText.setColor(Color.parseColor("#febf9c"));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(widthSpecMode == MeasureSpec.AT_MOST && heightSpecMode == MeasureSpec.AT_MOST){
            setMeasuredDimension(2* strokeWidth +  wStart + wSize  + 20 + paddingLeft + textSize *
                    boxText.length(),hStart + hSize + paddingTop + paddingBottom + 2 * strokeWidth);
            Log.d(TAG, wStart+" "+ wSize +" "+ textSize * boxText.length() + "   " +boxText.length());
        }else if(widthSpecMode == MeasureSpec.AT_MOST){
            setMeasuredDimension(2* strokeWidth +wStart + wSize + paddingLeft + 20 +  textSize *
                    boxText.length(),widthSpecSize);
    ...
    private void animationStart(){
        if(isHook) {
            ...
            animatorSet.playTogether(valueAnimator1,valueAnimator2,valueAnimator3,valueAnimator4,
                    valueAnimatorBorder,valueAnimator);
