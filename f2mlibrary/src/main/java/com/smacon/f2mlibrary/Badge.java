...
public class Badge extends FrameLayout {
    ...
    public Badge(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs, R.styleable.Badge, 0, 0);
        try {
            mBadgeTextColor = a.getColor(R.styleable.Badge_nbTextColor, DEFAULT_TEXT_COLOR);
            mTvBadgeText.setTextColor(mBadgeTextColor);

            mBadgeTextSize = a.getDimensionPixelSize(R.styleable.Badge_nbTextSize, (int)dpToPx(DEFAULT_TEXT_SIZE));
            mTvBadgeText.setTextSize(TypedValue.COMPLEX_UNIT_PX, mBadgeTextSize);

            mAnimationEnabled = a.getBoolean(R.styleable.Badge_nbAnimationEnabled, DEFAULT_ANIMATION_ENABLED);

            mAnimationDuration = a.getInt(R.styleable.Badge_nbAnimationDuration, DEFAULT_ANIMATION_DURATION);
            Drawable badgeBackground = a.getDrawable(R.styleable.Badge_nbBackground);
            if (badgeBackground != null) {
                mIvBadgeBg.setImageDrawable(badgeBackground);
            }

            mMaxTextLength = a.getInt(R.styleable.Badge_nbMaxTextLength, DEFAULT_MAX_TEXT_LENGTH);
            mEllipsizeText = a.getString(R.styleable.Badge_nbEllipsizeText);
